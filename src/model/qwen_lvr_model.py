...
class QwenWithLVR(Qwen2_5_VLForConditionalGeneration):
    def __init__(self, config):
        ...
        # Defensive getattr: probe / faithfulness scripts load
        # checkpoints outside train_lvr.py's discipline of always setting
        # these fields on config before from_pretrained.
        if getattr(config, "lvr_head", False):
            self._init_lvr_head(getattr(config, "lvr_head_type", "simple"))
    ...
    def generate(
        ...
        plvr_target_only: Optional[bool]=None,
        **kwargs,
        ...
        if plvr_target_only is None and hasattr(generation_config, 'plvr_target_only'):
            plvr_target_only = generation_config.plvr_target_only
        if plvr_target_only is None:
        ...
        if decoding_strategy == "latent":
            ...
            if getattr(self.config, 'plvr_mode', False):
                result = self._lvr_decoding_by_steps_plvr(
                    input_ids,
                    logits_processor=prepared_logits_processor,
                    stopping_criteria=prepared_stopping_criteria,
                    generation_config=generation_config,
                    ...
                    streamer=streamer,
                    lvr_steps=lvr_steps,
                    plvr_target_only=plvr_target_only,
                    **model_kwargs,)
            else:
...
# LVR docoding logic
    def _lvr_deocding_by_steps(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            last_tokens = input_ids[:,-1]
            lvr_start_switch = (last_tokens == self.config.lvr_start_id).to(device=input_ids.device)
            # P-LVR: also trigger on ctx_start and tgt_start (each stage gets its own fresh quota)
    ...
    def _lvr_decoding_by_steps_plvr(
        self,
        input_ids: torch.LongTensor,
        logits_processor: LogitsProcessorList,
        stopping_criteria: StoppingCriteriaList,
        ...
        streamer: Optional["BaseStreamer"],
        lvr_steps: List[int],
        plvr_target_only: bool = False,
        **model_kwargs,
    ) -> Union[GenerateNonBeamOutput, torch.LongTensor]:
        """P-LVR decoding: handles <|ctx_start|> and <|tgt_start|> stage triggers
        with separate step quotas, replacing the standard <|lvr_start|> trigger.

        When `plvr_target_only=True`, ctx markers are still generated as text tokens,
        but latent-mode decoding is only activated for the tgt stage."""
        # init values
        pad_token_id = generation_config._pad_token_tensor
        output_attentions = generation_config.output_attentions
        output_hidden_states = generation_config.output_hidden_states
        ...
        # Disable gradient checkpointing during generation: it disables KV cache in decoder
        # layers (incompatible), causing flash attention to see decode-step position_ids with
        # no zeros and fail. Generation runs under torch.no_grad() so gc serves no purpose.
        ...
        # P-LVR: separate ctx and tgt mode switches and quotas
        # ctx gets 2x the steps of tgt (lvr_steps refers to tgt)
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # P-LVR mode switch: trigger on <|ctx_start|> and <|tgt_start|> separately
            last_tokens = input_ids[:, -1]

            # --- ctx stage (2x tgt quota) ---
            ...
            # Combined LVR mode switch passed to forward pass
        ...
        # Re-enable gradient checkpointing if it was on before generation
        if gc_was_enabled:
