...
LOG_DIR="${WORKSPACE}/nlvr/logs"
mkdir -p $LOG_DIR
exec > >(tee "$LOG_DIR/nlvr_stage1_3b_$(date '+%Y%m%d_%H%M%S').log") 2>&1

# model configs
MODEL_NAME="Qwen/Qwen2.5-VL-3B-Instruct"
export WANDB_PROJECT="NLVR-Qwen25-VL-3B-SFT-STAGE-1-450k"
...
LAMBDA_LVR=0.1
NLVR_MODE=True
NLVR_NOISE_SCALE=0.3

MAX_TOKEN=5120
MIN_TOKEN=128

RUN_NAME="NLVRStage1_${LVR_LOSS_FCT}LVRLossLambda${LAMBDA_LVR}_Noise${NLVR_NOISE_SCALE}"
OUTPUT_DIR="${WORKSPACE}/nlvr/checkpoints/stage1/"
...
deepspeed src/train/train_lvr.py \
    ...
    --nlvr_mode $NLVR_MODE \
    --nlvr_noise_scale $NLVR_NOISE_SCALE \
